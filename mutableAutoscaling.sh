...
if [ $instanceId != i-005420df4c8559b2a ]
then
			exit 0
	else
	...
	then
		 echo "vani asg does not exists"
			aws autoscaling create-auto-scaling-group --auto-scaling-group-name vaniMutableAsg --launch-configuration-name $lcName --min-size 2 --max-size 4 --load-balancer-names mutable-lb --vpc-zone-identifier subnet-a5827ec2  --vpc-zone-identifier subnet-95b857bb --region us-east-1
	else
		  
	echo "vani asg exists"
			aws autoscaling update-auto-scaling-group --auto-scaling-group-name vaniMutableAsg --launch-configuration-name $lcName --min-size 0 --max-size 4 --vpc-zone-identifier subnet-a5827ec2 --vpc-zone-identifier subnet-95b857bb --termination-policies "OldestLaunchConfiguration"  --region us-east-1
